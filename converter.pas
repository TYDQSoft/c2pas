...
begin
 ...
 if(pas=false) then
  ...
  begin
   ...
   if(i>21) then c_string_is_operator:=0 else c_string_is_operator:=i;
  end;
end;
...
begin
 ...
 while(i<=code.Count) do
  begin
   ...
   if(layer=0) and (code.item[i-1]='operator') then break;
   inc(i);
  end;
...
begin
 i:=1;
 layer:=0; count:=0;
 if(code.count>1) and (code.item[1]='(') then
  begin
   exit(false);
  end
...
procedure condition_rehandle(var code:c_string);
var i,j,len,layer,layer2:SizeInt;
    tempcstr:c_string;
    bool:boolean;
begin
 i:=1; len:=code.count; layer:=0; layer2:=0;
 tempcstr:=c_string_generate_from_string('==0',false);
 while(i<=len)do
  begin
   if(code.item[i-1]='(') then inc(layer)
   else if(code.item[i-1]=')') then dec(layer)
   else
    begin
     layer2:=layer; j:=i; bool:=false;
     while(j<=code.count)do
      begin
       if(code.item[j-1]='(') then inc(layer2);
       if(code.item[j-1]=')') then dec(layer2);
...
begin
 ...
 if(Code.Count<=0) then
  ...
  begin
   ...
   condition_rehandle(ifstate^.condition);
     ...
     while(i<=extcode.count)do
      begin
       if(extcode.item[i-1]='__attribute__') then
        ...
        begin
         structstate^.structdefname:=extcode.item[i-1];
         c_string_delete_item(extcode,i,1); continue;
        end;
       inc(i);
      end;
  ...
  begin
   ...
   while(i<=code.Count)do
    begin
     if(code.item[i-1]='(') then
     ...
     else if(code.item[i-1]='operator') then
      begin
      ...
      begin
       ...
       if(i<code.count) and (code.item[i-1]<>'*') and (code.item[i]<>'*') then
    ...
    begin
     if(i<code.count) and (code.item[i]='::') then
      ...
      begin
       ...
       then classfuncstate^.funcname:=code.item[i-1];
       j:=i+1; k:=i+1; layer:=1;
       while(j<=code.count) do
        begin
         if(code.item[j-1]='(') then inc(layer);
   ...
   while(i<=code.Count)do
    begin
     if(code.item[i-1]='struct') then
      ...
      begin
       structstate^.structdefname:=code.item[i-1];
       c_string_delete_item(code,i,1); continue;
      end;
     inc(i);
    end;
 ...
 and (c_string_check_function(code)=false) and (bool=true) and (specindex=1) then
  ...
  begin
   ...
   k:=c_string_divide_type_and_name(code,1);
   while(i<=k)do
    begin
...
begin
 ...
 while(i<=tempexp.count)do
  begin
   ...
   else if(tempexp.item[i-1]='NULL') then
    begin
  ...
  begin
   if(value.item[i-1]='#') then
    begin
     ...
     while(k<=tempcstr.count)do
      begin
       if(LowerCase(tempcstr.item[k-1])='Defined') then
        begin
...
begin
 ...
 if(orgtree^.treetype=c_node_asm) then
  ...
  begin
   ...
   if(Pc_enum(orgtree^.content)^.enumdefname<>'') then
    begin
     enumstate^.enumname:=Pc_enum(orgtree^.content)^.enumdefname;
    end
  ...
  begin
   New(recstate);
   if(Pc_struct(orgtree^.content)^.structdefname<>'') then
   recstate^.recordname:=Pc_struct(orgtree^.content)^.structdefname
...
begin
 ...
 for i:=count downto 1 do
  begin
   ...
   if(j<=ifdefcond.count) then continue;
  ...
  begin
   ...
   for i:=1 to roottree^.count do
    begin
     ...
     if(basetree^.treetype=pas_node_ifdef) then
      begin
       inc(list.count);
     ...
     else if(basetree^.treetype=pas_node_define) and (specindex=0) then
      begin
       inc(list.count);
       SetLength(list.tree,length(list.tree)+1);
       pas_tree_copy(basetree,list.tree[length(list.tree)-1]);
     ...
     else if(basetree^.treetype=pas_node_class) and (specindex=7) then
      begin
       inc(list.count);
       SetLength(list.tree,length(list.tree)+1);
       pas_tree_copy(basetree,list.tree[length(list.tree)-1]);
...
begin
 ...
 if(desttree=nil) then
  begin
   ...
   pas_tree_get_list(orgtree,tempfuncdlist,6);
   ...
   tempfuncdlist.count:=length(tempfuncdlist.tree);
   ...
   for i:=1 to tempfunclist.count do
    begin
     ...
     temptree^.child:=nil;
     temptree^.count:=0;
     if(tempfunclist.tree[i-1]^.treetype<>pas_node_ifdef)
...
begin
 ...
 if(temptree^.treetype=pas_node_function) or (temptree^.treetype=pas_node_operator) then
  ...
  begin
   if(pastree^.treetype<>pas_node_root) and (pastree^.treetype<>pas_node_class) then continue;
  end
   ...
   if(Ppas_class_function(temptree^.content)^.resulttype='') then
    begin
     if(temptree^.parent^.treetype=pas_node_class) then
      begin
       if(Ppas_class_function(temptree^.content)^.isconstructor) then
       res:=res+'constructor '+Ppas_class_function(temptree^.content)^.hostname+'.'+
       Ppas_class_function(temptree^.content)^.funcname
       else if(Ppas_class_function(temptree^.content)^.isdestructor) then
